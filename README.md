...
- SSH サーバ（Ed25519 鍵交換、パスワード認証、PTY セッション）
- host (macOS) から `ssh -p 10022 root@127.0.0.1` で guest shell に接続可能
...
2006-2007 年に実機（3.5 インチ FDD 付き i486 PC）向けに開発した時点で、以下が動作していました。
...
- システムコールインターフェース（`syscall.c` / `sys_core.S`）
- ビルドツール（`kmkfs` ディスクイメージ作成、`getsize`）

## 2025 年以降に追加したもの

このリポジトリを GitHub に上げた後、以下を新たに実装しました。

- QEMU 上での安定動作とテスト基盤（host unit test + QEMU smoke test）
- POSIX ソケット API（TCP / UDP / ICMP raw socket、`ping` コマンド）
- framebuffer グラフィカルターミナル（VT100/ANSI パーサ、スクロールバック）
- TTY/PTY サブシステム
- shell のパイプ `|` とリダイレクト `>` `<` `>>`
- `vi` エディタ（modal editing、undo/redo、ファイル保存）
- UTF-8 表示と日本語 IME（ひらがな / カタカナ / 漢字変換、Mozc 辞書）
- UDEV Gothic ビットマップフォント生成
- メモリスケーリング（128MB〜1024MB の動的メモリ管理）
- サーバランタイム（HTTP / Admin / Debug Shell / SSH サーバ）
- SSH サーバ（Ed25519 鍵交換、パスワード認証、PTY セッション）
- 暗号ライブラリ（AES、SHA-256、Ed25519 / TweetNaCl）
- Docker コンテナデプロイ（ARM64 / x86_64 対応）
- ext3 大容量ファイルサポート
