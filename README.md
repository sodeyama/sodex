...
- コマンド群: `ls`（カラー対応）、`cat`、`touch`、`mkdir`、`rm`、`rmdir`、`mv`、`cd`、`pwd`、`ps`、`kill`、`sleep`
...
- agent built-in file tools（`read_file` `write_file` `list_dir`）
...
- QEMU integration test（boot、メモリスケーリング、FS、shell I/O、vi、IME、SSH、HTTPS、websearch、webfetch、agent）
