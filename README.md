...
- `sxi`（`sx` source interpreter: `fn` / `let` / block / `if` / `while` / `return`、`str` / `bool` / `i32`、`io` / `fs` / `proc` / `json` / `text`、relative `import`、`--check`、`-e`、REPL、runtime stack trace）
- guest image に `/home/user/sx-examples/README.md` と `hello.sx` / `import_main.sx` / `json_report.sx` などの `sx` サンプルを同梱
...
`make test-agent-full` は `fetch_url` tool を含む agent integration を QEMU で通します。
