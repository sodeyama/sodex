...
- `sodex /> pwd` を打って `/` が返るところまでは確認済み
- transport / KEX / `NEWKEYS` / `password` auth / `session` / `pty-req` / `shell` / `window-change` は実装済み

まだ残っている不安定点は次の 2 つ。

- host 側で prompt が見えない run があり、`ssh -tt` の見え方がまだ安定していない
- 同一 session で `pwd` に続けて `ls` を打つと `shell_exit` で閉じる run があり、multi-command session が未安定
...
- [ ] `pwd` / `ls` / `exit` を同一 session で安定して通せる
