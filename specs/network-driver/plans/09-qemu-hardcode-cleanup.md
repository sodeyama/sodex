...
- `src/drivers/ne2000.c` に accessor を追加
- `src/net/netmain.c` は `ne2000_read_isr()` / `ne2000_ack_isr()` を使う形へ変更
- `src/test/ktest.c` は `ne2000_enable_interrupts()` / `ne2000_read_bnry()` を使う形へ変更
- `0xC100` の直叩きは network-driver の主要コードから外した
...
- [x] `0xC100` の重複参照が整理されている
- [x] NIC ベースアドレスの変更点が追いやすい
- [x] QEMU 前提値が driver / test / doc のどこに属するか明確
