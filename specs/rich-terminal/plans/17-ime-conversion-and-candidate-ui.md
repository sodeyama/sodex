...
3. `term` の入力経路に変換開始、候補移動、確定、キャンセルの action を追加する
4. `term` overlay に候補 UI を追加し、preedit と候補列を見分けられるようにする
5. shell と `vi` で、候補選択後の確定 UTF-8 が保存まで通ることを確認する
6. host test で辞書 lookup、候補遷移、キャンセル、確定を固定する
7. QEMU smoke で変換、保存、再表示を固定する
