...
typedef struct _RawDevice {
  int (*raw_read)(u_int32_t lba, u_int32_t sects, void* buf);
  int (*raw_write)(u_int32_t lba, u_int32_t sects, void* buf);
} RawDevice;

#define DIRTY

PUBLIC ext3_dentry *rootdir;
PUBLIC ext3_dirty *rootdirty;
...
struct files_struct gtask; // for test
//RawDevice *rawdev = NULL;
RawDevice rawdev;
