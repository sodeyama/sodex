...
2026-03-15 時点の到達点は次のとおり。

- host の `OpenSSH` client から
  `ssh -tt -F /dev/null -o PreferredAuthentications=password -o PubkeyAuthentication=no -p <hostfwd_port> root@127.0.0.1`
  で guest `eshell` まで到達できる
- 同一 session で `backspace` 付き `pwd`、`ls`、`cat` + `Ctrl-C`、`exit` を打って prompt 復帰と clean close まで確認済み
- `run_qemu_ssh_smoke.py` と `make test-qemu-ssh` で login / `backspace` 付き `pwd` / `ls` / `cat` + `Ctrl-C` / `exit` / wrong password / reconnect / client disconnect を固定できた
- transport / KEX / `NEWKEYS` / `password` auth / `session` / `pty-req` / `shell` / `window-change` は実装済み
- `ssh -tt` で崩れていた改行は `CRLF` 正規化で解消し、prompt 未表示 run、`backspace` 崩れ、foreground command への `Ctrl-C` 伝播も改善した

まだ残っている主な未完了は次の 4 つ。

- auth failure の retry 制限が SSH login path にまだ統合し切れていない
- host key fingerprint と known_hosts を固定した手順をまだ整えていない
- `ssh -p ... root@127.0.0.1` の追加オプションなし login はまだ未確認
- packet / auth policy / channel state machine の host test 拡充がまだ未完了
...
- `ssh` client で login
- `backspace` 付き `pwd` / `ls` / `cat` + `Ctrl-C` / `exit` / client disconnect で shell relay を確認
- 誤 password の拒否
- reconnect
- `window-change` の反映
...
- [x] `password` auth, `session`, `pty-req`, `shell`, `window-change` が成立する
- [x] client disconnect ベースの切断と再接続が安定する
- [ ] `exec`, `subsystem`, forwarding, 複数 channel を reject できる
- [x] `pwd` / `ls` / `exit` を同一 session で安定して通せる
