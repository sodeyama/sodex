...
| M14: フル IME 辞書 | 18 | guest 内 IME が大規模辞書 blob と cache を使い、基本語彙を広く候補提示できる |
| M15: large dictionary 向け fs 基盤 | 19 | ext3 / `kmkfs` が multi-MiB 単一 file を扱え、大きい辞書 blob を fs 制約で諦めなくてよくなる |

## 現在の到達点

- M13 は完了済み、M14 は辞書 blob / cache / paging 基盤まで実装済み、M15 はこの branch で実装済み
...
- source 辞書からの blob 生成、on-disk lookup、small cache、fallback、候補 paging、host/QEMU smoke は実装済みで、Plan 18 の残りは外部辞書 source / license の確定が中心になっている
...
- Plan 18 は大規模辞書の土台まで進み、残りは外部辞書 source と語彙拡張の統合が中心になっている
