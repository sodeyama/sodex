...
| 15 | [15-vi-basic-commands.md](plans/15-vi-basic-commands.md) | `vi` の基本移動・削除・追記コマンドを拡張し、通常編集を実用段階へ引き上げる | 09, 12, 13, 14 | `x`, `dd`, `dw`, `0`, `$`, `w`, `b`, `e`, `gg`, `G`, `a`, `A`, `I`, `o`, `O` などで編集できる |
| 16 | [16-terminal-usability-and-hardening.md](plans/16-terminal-usability-and-hardening.md) | terminal / shell / `vi` の回帰基盤、待機、parser、編集体験を実用段階へ引き上げる | 09, 11, 13, 15 | 集約 host test が通り、`term` の idle / resize、`eshell` の quoting / 複数 pipeline、`vi` の alternate screen / undo / 検索 / visual mode が揃う |
| 17 | [17-ime-conversion-and-candidate-ui.md](plans/17-ime-conversion-and-candidate-ui.md) | guest 内 IME に漢字変換、辞書 lookup、候補 UI を追加する | 09, 13, 14, 16 | shell / `vi` で候補付き漢字変換を行い、そのまま保存できる |
| 18 | [18-full-ime-dictionary.md](plans/18-full-ime-dictionary.md) | IME を最小固定辞書から大規模辞書 + on-disk lookup + cache へ拡張する | 17, `specs/memory-scaling` | 基本語彙を広く引ける辞書 blob を shell / `vi` で実用速度と memory budget 内で扱える |
| 19 | [19-ext3fs-large-file-support.md](plans/19-ext3fs-large-file-support.md) | ext3 / `kmkfs` を multi-MiB 単一 file と double indirect へ拡張し、大きい辞書 blob の搭載制約を外す | 10, 17, `specs/memory-scaling` | 5MiB 級 file を image へ載せ、guest userland から境界 block まで検証できる |
| 20 | [20-shell-tab-completion.md](plans/20-shell-tab-completion.md) | shell prompt 上の path tab completion と候補 UI を `term` 側で追加する | 16, 17 | `cat hoge_` のような入力に対して、一意補完、共通 prefix 延長、候補巡回が成立する |
| 21 | [21-sentence-ime-conversion.md](plans/21-sentence-ime-conversion.md) | IME を単一読みの候補置換から連文節 / 全文変換へ拡張する | 18, 20 | shell / `vi` で未確定入力全体を変換し、`Enter` で一括確定できる |
| 22 | [22-shell-stderr-redirection-and-pager.md](plans/22-shell-stderr-redirection-and-pager.md) | shell の fd 指定 redirection と `more` / `less` 相当の pager を追加する | 11, 13, 16 | `2>&1` 系の I/O 合成と page 単位の長文閲覧が guest 内で成立する |
| 23 | [23-unix-text-commands-and-agent-usage.md](plans/23-unix-text-commands-and-agent-usage.md) | `find`, `sort`, `uniq`, `wc`, `head`, `tail`, `grep`, `sed`, `awk`, `cut`, `tr`, `diff`, `tee` を guest と agent の両方で使えるようにする | 11, 13, 16 | guest 内で探す・絞る・数える・変換する・比較する基本 text workflow が成立する |
| 24 | [24-unix-text-tools-command-split.md](plans/24-unix-text-tools-command-split.md) | Unix text command 群の本体を `command` 側へ戻し、共有 helper だけを `libc` へ残す | 23 | command ごとに責務が追え、host/QEMU 回帰を保ったまま monolith を解消できる |
...
| M11: vi 基本編集の実用化 | 15 | normal mode の基本移動、削除、追記で既存ファイルを日常的に編集できる |
| M12: terminal / shell / vi の実用化と堅牢化 | 16 | rich terminal 系の回帰基盤が green に戻り、`term` の idle / resize、`eshell` の quoting / 複数 pipeline、`vi` の alternate screen / undo / 検索 / visual mode が揃う |
| M13: IME 漢字変換と候補 UI | 17 | guest 内 IME が候補付き漢字変換を持ち、shell / `vi` で確定保存できる |
| M14: フル IME 辞書 | 18 | guest 内 IME が大規模辞書 blob と cache を使い、Mozc 由来語彙を含む広い基本語彙を候補提示できる |
| M15: large dictionary 向け fs 基盤 | 19 | ext3 / `kmkfs` が multi-MiB 単一 file を扱え、大きい辞書 blob を fs 制約で諦めなくてよくなる |
| M16: IME の実用速度調整 | 18 | lookup、result cache、overlay、framebuffer 描画の hot path を詰め、変換時の待ちを減らせる |
| M17: shell のタブ補完 | 20 | `term` 上の shell prompt で path 補完、候補巡回、overlay 表示が成立し、`ssh -tt` 経由でも inline 補完できる |
| M18: IME の連文節 / 全文変換 | 21 | guest 内 IME が未確定入力全体を clause 列として扱い、文節調整後に `Enter` で一括確定できる |
| M19: shell の stderr redirection と pager | 22 | `2>`, `2>&1` などの stream 制御と `more` / `less` 相当の長文閲覧が guest 内で成立する |
| M20: Unix 系 text command 群 | 23 | `find`, `sort`, `uniq`, `wc`, `head`, `tail`, `grep`, `sed`, `awk`, `cut`, `tr`, `diff`, `tee` が shell / agent から使える |

## 現在の到達点

- M13, M14, M15, M17, M18, M20 は実装済み
...
- `term` 上の shell prompt で path の `Tab` / `Shift+Tab` 補完と候補 overlay が動作し、`cat ` のような空 path token でもその directory の候補を巡回できる
- `ssh -tt` で開いた shell prompt でも path の `Tab` 補完が inline で動作する
- host test と QEMU smoke で CRUD、shell I/O、`vi`、UTF-8 保存導線を固定している
- `term` で `Ctrl+Space` または `半角/全角` により `latin` / `hiragana` を即時トグルでき、`変換` は `hiragana`、`無変換` は `latin`、`かな` は `hiragana`、`Shift+かな` は `katakana` を直接選べる
...
- `make -C tests test` が rich terminal 系を含めて再び通る状態へ戻っている
- `term` は idle 時に `sleep_ticks(1)` で待機し、graphics mode 側でも viewport 再計算と winsize 再通知を通している
- `eshell` は quoting、escape、複数段 pipeline、`>>` を扱い、host/QEMU smoke で固定されている
- `2>`, `2>&1`, `more`, `less` のような advanced redirection / pager workflow は未対応で、Plan 22 の対象である
- shell / agent から `find`, `sort`, `uniq`, `wc`, `head`, `tail`, `grep`, `sed`, `awk`, `cut`, `tr`, `diff`, `tee` を使え、host test と QEMU smoke で代表 workflow を固定している
- Unix text command 群の本体は `src/usr/command/*.c` に分割され、共有 helper は `src/usr/lib/libc/unix_text_tool_lib.c` に集約している
- `vi` は alternate screen、undo/redo、`/` / `?` / `n` / `N`、char-wise / line-wise visual mode を扱い、QEMU smoke でも shell 復帰まで確認できる
- `Space` / `変換` で漢字変換へ入り、候補 overlay、候補巡回、確定、キャンセルを shell / `vi` の両方で扱える
- 固定辞書ベースの最小漢字変換と候補 UI は host test と QEMU smoke で固定している
- source 辞書からの blob 生成、on-disk lookup、small cache、fallback、候補 paging、host/QEMU smoke は実装済み
- Plan 18 の辞書 source は Mozc `dictionary_oss` と手製補助語彙に確定し、build 時に約 11 万読み / 14 万候補規模の source TSV と約 5.2MiB の blob を生成できる
- ext3 / `kmkfs` は double indirect まで対応し、5MiB fixture を image に載せて guest userland から境界 block を検証できる
- `半角/全角` は `term` 上では IME toggle を優先するため、US 配列の `` ` `` / `~` をそのまま打ちたい場合は別キー経路が必要
- `vi` の normal mode で `0`, `^`, `$`, `w`, `b`, `e`, `gg`, `G`, `x`, `X`, `dd`, `D`, `dw`, `db`, `de`, `d0`, `d$`, `a`, `A`, `I`, `o`, `O` を扱える
- host test で `vi_buffer` の移動・削除・行追加を固定し、QEMU smoke でも基本編集フローを確認できる形にしている
- Plan 16 は完了し、回帰基盤、`term` の idle / resize、`eshell` の parser、`vi` の実用機能まで固定できた
- Plan 17 も完了し、かな直接入力の上に最小漢字変換と候補 UI を重ねた
- Plan 18 は完了し、Mozc 由来の大規模辞書 source を build へ統合した
- Plan 19 は完了し、large dictionary blob を載せるための fs blocker を外した
- Plan 21 は完了し、IME が未確定入力全体を clause 列として保持し、focused clause の候補調整、境界調整、`Enter` 優先 commit を扱える
- Plan 18 の tuning として reading fingerprint、4KiB cache、recent-reading cache、overlay 差分描画、renderer 高速化を追加した
- Plan 21 の host test は通過しているが、`make -C src test-qemu-ime` の安定化は boot が rescue 経由になる既知問題のため GitHub Issue #24 で追跡中である
...
10. 次に `term` 常駐 IME と UTF-8 入力経路を入れ、日本語直接入力へ広げる
11. その上で回帰基盤、idle wait、shell parser、`vi` 実用機能を整え、日常利用の粗さを下げる
12. 次にかな IME の上へ漢字変換、辞書 lookup、候補 UI を載せる
13. さらに memory scaling の後で、大規模辞書 blob と cache を載せて IME を実用語彙へ広げる
14. その次に shell prompt 上の tab completion を `term` 側へ足し、日常操作の入力コストを下げる
15. 最後に IME を単一読みの候補置換から連文節 / 全文変換へ広げ、`Enter` 一括確定へ寄せる
16. その後に shell の fd 指定 redirection と `more` / `less` 相当の pager を足し、長文閲覧と log 保存を guest 内で閉じる
17. その次に Unix 系 text command 群を追加し、shell / agent の調査・加工・比較 workflow を guest 内で閉じる
...
- `16` は `15` 完了後に着手し、test 修復、`term`、shell、`vi` を並行に薄く進められる
- `17` は `16` の回帰基盤が戻った後に着手し、IME の辞書 / 候補 UI へ集中する
- `18` は `17` 完了後に着手し、`specs/memory-scaling` の allocator headroom と QEMU memory knob を前提に大規模辞書へ進める
- `20` は `16`, `17` 完了後に着手し、path match helper と `term` overlay を並行に進められる
- `21` は `18` の辞書 blob と `20` の overlay 共存前提が揃った後に着手し、composition state、segmenter、`term` 入力系を並行に薄く進められる
- `22` は `11`, `13`, `16` 完了後なら IME 系と比較的独立に進めやすく、shell parser / executor、pager command、QEMU smoke を並行に薄く進められる
- `23` は `11`, `13`, `16` 完了後なら比較的独立に進めやすく、共通 text helper、各 command、本数を絞った QEMU smoke を並行に薄く進められる
...
- `2>`, `2>&1`, `more`, `less` は Plan 22 で扱う
  - まず stdout / stderr の分離と左から右の fd 評価順を固め、その後に pager を載せる
...
- IME の連文節 / 全文変換は Plan 21 で扱う
  - 単一 `reading` と backspace 置換を基点にしつつ、未確定入力全体と clause state を導入する
  - 既確定 text の再変換は app 協調が前提なので別レイヤとして扱う
...
- `term` の idle wait を最小 syscall でどう表現するか
- graphics mode の resize 検出を polling、通知、または ioctl 風 API のどれで行くか
- shell の quoting / escape をどこまで POSIX 互換へ寄せるか
- `vi` の undo 履歴を operation log と buffer snapshot のどちらへ寄せるか
- IME の連文節変換で composition を PTY 未送信 buffer に寄せるか、既存の backspace 置換を段階移行で残すか
- Mozc 由来 cost を blob v3 へどこまで残すか
  - candidate cost だけで足りるか、品詞や接続 cost まで持つか
- 再変換を `term` 単独で扱わず、`vi` など app 協調 API としてどう切るか
