...
2026-03-22 に Plan 07 を実装し、shell を「service script 実行器」から
より一般的な script 実行系へ広げた。
...
- host unit test と、boot-time `sh` / redirected `eshell` を使う QEMU smoke

同日の追加調査では、次の「普段使いの shell」としての不足も切り出した。
...
これらは completion の有無とは別の shell 本体機能なので、
Plan 08 として lookup / history / expansion をまとめて扱う。

2026-03-22 に Plan 08 を実装し、次を追加した。

- `alias`, `unalias`
- `type`, `command -v`
- current shell session の `history`, `!!`, `!prefix`
- `~` 展開
- basic glob (`*`, `?`)
- host unit test `test_shell_usability`
- boot-time `sh` と redirected `eshell` を使う QEMU smoke `test-qemu-shell-usability`

completion は引き続き `term` / `shell_completion.c` 側の機能として維持し、
今回の scope には `hash`, shell function, 永続 history, command substitution,
arithmetic expansion, brace expansion は含めない。
...
| 07 | [plans/07-programmable-shell-control-flow.md](plans/07-programmable-shell-control-flow.md) | `if` / loop / `test` と対話継続入力を加え、programmable shell を成立させる | 02, 03, 04, 06 |
| 08 | [plans/08-shell-usability-and-expansion.md](plans/08-shell-usability-and-expansion.md) | `alias` / `history` / `type` / `command -v` と `~` / glob を加え、日常利用寄りの shell にする | 03, 07 |
...
- compound command を flat な pipeline model へ雑に継ぎ足すと parser/executor が壊れやすい
- `eshell` の継続入力は prompt、Tab completion、`term` の canonical 入力との整合が必要
- `test` / `[` の subset を曖昧にすると `if` 条件の移植性が低くなりやすい
...
- [ ] `if` / `elif` / `else` / `fi` と `for` / `while` / `until` を `sh` / `eshell` の両方で扱える
- [ ] `test` / `[` の最小 subset で文字列比較、空判定、代表 file 判定を書ける
- [ ] interactive `eshell` が継続 prompt 付きの multi-line compound command を入力できる
- [ ] host test と QEMU smoke で条件分岐、繰り返し、変数利用、`break` / `continue` を固定する
