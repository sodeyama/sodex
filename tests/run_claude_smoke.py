#!/usr/bin/env python3
"""
QEMU Claude smoke test runner (Phase C).

1. Generates TLS certs (if missing)
2. Starts mock Claude SSE server with TLS
3. Starts mock HTTP server (for Phase A tests that still run)
4. Rebuilds with overlay that runs 'claude' command
5. Launches QEMU
6. Checks serial output for PASS/FAIL
"""
...
def main() -> int:
    ...
    serial_log = logdir / "claude_test_serial.log"
    qemu_log = logdir / "claude_test_qemu_debug.log"
    mock_log = logdir / "claude_test_mock.log"
    claude_log = logdir / "claude_test_claude_mock.log"
    monitor_log = logdir / "claude_test_monitor.log"
    monitor_sock = logdir / "claude_test_monitor.sock"

    for p in (serial_log, qemu_log, mock_log, claude_log, monitor_log, monitor_sock):
        if p.exists():
            p.unlink()

    # Build with claude overlay
    print("Building with claude overlay...")
    ...
    # Create rootfs overlay that runs 'claude' command
    default_overlay = repo_root / "src" / "rootfs-overlay"
    overlay_dir = logdir / "claude-rootfs-overlay"
    ...
    # Start mock HTTP server (for backward compat tests)
    ...
    tls_proc = None  # Not used separately here
    try:
        if not wait_for_port("127.0.0.1", MOCK_PORT, timeout=5.0):
            ...
            claude_proc.kill()
            return 1
        print(f"Mock HTTP server on port {MOCK_PORT}")

        if not wait_for_port("127.0.0.1", CLAUDE_TLS_PORT, timeout=5.0):
            print("WARNING: Mock Claude server failed to start", file=sys.stderr)
        ...
        # Report
        print("")
        print("=== CLAUDE SMOKE TEST RESULTS ===")
